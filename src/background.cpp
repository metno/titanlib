...
vec titanlib::background(const vec& elevs, const vec& values, int num_min_prof, float min_elev_diff, float value_minp, float value_maxp, BackgroundType background_type, const vec& external_background_values, const ivec& indices_global_outer, bool debug) {
    ...
    if( background_type == titanlib::VerticalProfile) {
        ...
        bvalues = titanlib::compute_vertical_profile(elevs, elevs, values, num_min_prof, min_elev_diff, debug);
    ...
    // set the range 
    for(int i=0; i<p; i++) {
        if(titanlib::is_valid(value_minp) && bvalues[i] < value_minp) bvalues[i] = value_minp;
        if(titanlib::is_valid(value_maxp) && bvalues[i] > value_maxp) bvalues[i] = value_maxp;
    }
    //
    return bvalues;
}

vec titanlib::compute_vertical_profile(const vec& elevs, const vec& oelevs, const vec& values, int num_min_prof, double min_elev_diff, bool debug) {
    if(elevs.size() == 0)
